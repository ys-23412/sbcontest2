...
# Adjust these imports to match your local structure
...
def _map_srd_tender_entry(tender_record: dict, params: dict, city_mapping: dict) -> dict:
    """
    Maps the parsed SRD tender data into the required system payload structure.
    """
    entry = {}
    ys_body = {}
    ...
    description = str(tender_record.get('description', ''))
    try:
    ...
    try:
        bid_opportunity = clean_title.split(' ', 1)[0]
        entry['ys_permit'] = bid_opportunity
    except Exception:
        entry['ys_permit'] = clean_title
        pass
    ...
    raw_project = re.sub(r'\s*-\s*', '-', clean_title).replace('–', '-')
    ys_body['ys_project'] = unidecode(raw_project[:97])
    
    ys_body['ys_sector'] = 'Public'
    ys_body['ys_reference'] = entry['ys_permit']
    ys_body['ys_tender_authority'] = 'Strathcona Regional District'
    ys_body['ys_documents_drawings_link'] = tender_record.get('Opportunity Url', '')
    
    # Build Enquiries string from CSV columns
    contact_name = str(tender_record.get('contact_name', ''))
    job_title = str(tender_record.get('job_title', ''))
    email = str(tender_record.get('email', ''))
    
    enquiries = []
    if job_title and job_title.lower() != 'nan':
    ...
    # Stage / Type mapping
    raw_type = str(tender_record.get('Type', 'Request for Proposals')).strip()
    ys_body['ys_stage'] = raw_type if raw_type and raw_type.lower() != 'nan' else 'Request for Proposals'

    # 3. Handle Dates
    # Parse Issue Date
    parsed_date_str = tender_record.get('Parsed Date')
    if pd.notna(parsed_date_str) and str(parsed_date_str).lower() != 'nat':
        try:
            entry['ys_date'] = datetime.strptime(str(parsed_date_str)[:10], "%Y-%m-%d").strftime('%Y-%m-%d')
        except ValueError:
    ...
    closing_date_str = str(tender_record.get('Closing date', ''))
    
    if closing_date_str and closing_date_str.lower() != 'nan':
        # Remove the pipe separator to help dateparser "Jul 29, 2026 | 2:00 pm" -> "Jul 29, 2026 2:00 pm"
        clean_closing = closing_date_str.replace('|', '').strip()
        parsed_date_close = dateparser.parse(clean_closing)
...
def process_and_send_srd_tenders(params: dict):
    """
    Maps, packages, and sends the extracted Strathcona Regional District (SRD)
    ...
    # Hardcoded values for Strathcona Regional District
    region_name = "Strathcona Regional District"
    city_name = "Strathcona RD"
    file_prefix = "srd_tender"
    ...
    # 2. Filter records by Posted Date (only process if posted today)
    for record in tender_records:
        parsed_date_str = str(record.get('Parsed Date', ''))
        
        record_date = None
        try:
            if parsed_date_str and parsed_date_str.lower() != 'nan' and parsed_date_str.lower() != 'nat':
                # Convert "2026-06-17" to date
                record_date = datetime.strptime(parsed_date_str[:10], "%Y-%m-%d").date()
            else:
            ...
            print(f"⚠️ Warning: Date parsing failed for record '{record.get('Title', 'Unknown')}'. Error: {e}")
            continue
    ...
    # 3. Map the Extracted Records
    for record in tender_records:
        try:
            mapped_result = _map_srd_tender_entry(record, params, city_mapping)
            ...
            traceback.print_exc()
            opp_id = record.get('Title', 'Unknown ID')
            print(f"⚠️ Failed to map SRD tender {opp_id}. Error: {e}")
    ...
    if total_found == 0:
        print("No new records matching today's date found. Exiting.")
        return

    # 4. Setup Payload & Save Locally (For Debugging)
    ...
    try:
        ...
        send_discord_message(
            message=f"🤖 {region_name} Harvester: Run Complete\nAutomated run finished processing {region_name} tenders. Exception occurred.",
            webhook_url=discord_webhook_url
        )

# main to read and send files
if __name__ == "__main__":
    main_csv = f"{FILE_DIR}/srd_enriched_bids.csv"
    if not os.path.exists(main_csv):
        ...
        if weekday == 5 or weekday == 6:
            ...
            exit(0)
        # if file srd_new_bids_raw.csv is available then just assume no new bids
        ...
        try:
            # We don't want to reupload the entire file if anything goes wrong
            process_and_send_srd_tenders(params)
        except Exception as e:
            ...
            try:
                ...
                send_discord_embed(
                    webhook_url=discord_webhook_url,
                    title="🤖 SRD Harvester: Failure",
                    description="Csv processing failed, csv should exist.",
                    fields={"💤 Status": "BAD THINGS HAPPENED"},
                    color=9807270 # Grey
                )
            except Exception as nested_e:
                print(f"❌ An unexpected error occurred in Discord failure relay: {nested_e}")